                    ...
                    <div class="col-12 pt-2 h-100 shadow rounded bg-white">
                        ...
                        <div class="text-sm text-gray-700">
                            ...
                            <p>     
                                ...
                                <span class="p-4 alert {{ $reservation->payment_status == 'Full Payment' ? 'alert-success' : '' }} {{ $reservation->payment_status == 'Down Payment' ? 'alert-warning' : '' }} {{ $reservation->payment_status == 'Cash on Delivery' ? 'alert-failed' : '' }} {{ $reservation->payment_status == 'Pay Online' ? 'alert-approved' : '' }}">
                        ...
                        <div class="flex justify-between items-center mt-2">
                            ...
                            @if(in_array($reservation->status, ['Approved', 'In Progress', 'Fulfilled']) && $reservation->payment_status !== 'Cash on Delivery')
                                <div class="dropstart">
                                        ...
                                        <form method="post" action="{{ route('receiptImage') }}" enctype="multipart/form-data">
                                            ...
                                            <div class="mb-1">
                                                ...
                                                <div class="input-group mb-3">
                                                    <input id="image-upload-input-{{ $reservation->id }}" type="file" name="images[]" accept="image/*" multiple required style="display: none;" onchange="previewImages(this, {{ $reservation->id }})">
                                                    <label for="image-upload-input-{{ $reservation->id }}" class="btn btn-secondary" style="font-size: 0.65rem; font-style: italic;">
                                                        <i class="fas fa-camera"></i> Choose Images
                                                    </label>
                                                </div>

                                                <div id="image-previews-{{ $reservation->id }}" class="d-flex mt-2"></div>
                                            </div>
                            ...
                            @if(in_array($reservation->status, ['Approved', 'In Progress']))
        ...
        function closeCancellationPolicyModal() {
            ...
            document.getElementById('cancellationPolicyModal').classList.add('hidden');
                            ...
                            @elseif($reservation->status == 'Pending')
                                <form action="{{ route('reservations.cancel', $reservation->id) }}" method="POST" class="inline-block flex flex-col items-center" >
                                    @csrf
                            ...
                            @elseif($reservation->status == 'Fulfilled' && !$reservation->rating && $reservation->receipt_image)
                                <div class="flex flex-col items-center">
                                    <button class="flex items-center  custom-red-icon rounded-lg text-black rate-btn" data-reservation-id="{{ $reservation->id }}" data-service-id="{{ $reservation->service_id }}" data-package-id="{{ $reservation->package_id }}">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="container">
                                                <form action="{{ url('charge') }}" method="post" id="paymentForm">
                                                    ...
                                                    @if($reservation->payment_status !== 'Cash on Delivery')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="service_rating" id="rating" value="0">
                </div>

                <!-- Package Rating (if applicable) -->
                <div class="form-group" id="packageRatingSection" style="display: none;">
                    <label>Food</label><br>
                    ...
                    <input type="hidden" name="package_rating" id="qualityRating" value="0">
                </div>
               
               <b><p><span id="overallRatingStars"></span>&nbsp;<span id="averageRating"></span></p></b>
...
$(document).ready(function () {
    ...
    $('.rate-btn').click(function () {
        ...
        if (packageId) {
            ...
            $('#packageRatingSection').show(); // Show package rating section
        } else {
            $('#packageId').val('');
            $('#packageRatingSection').hide(); // Hide package rating section if no package
        }
    ...
    // Package rating click event (only visible if package is selected)
    ...
    function updateOverallRating() {
        ...
        var totalRating =
            (serviceRating > 0 && qualityRating > 0)
                ? (serviceRating + qualityRating) / 2
                : serviceRating;

        // Update UI with the overall rating
    ...
    $('#submitRating').click(function () {
        ...
        var packageId = $('#packageId').val(); // Package ID (if applicable)
        var packageRating = $('#qualityRating').val(); // Rating for the package (if applicable)
        var comment = $('#comment').val().trim();

        // Validate service rating
        if (serviceRating === '0') {
            alert('Please rate the service. It is a required field.');
            return; // Prevent submission if service rating is missing
        }

        // Handle package rating
        if (packageId === '' || packageRating === '0') {
            packageRating = null; // Set to null if no package or no rating
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                package_id: packageId || null, // Pass null if no package is selected
